        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="mt-5 grid gap-6 md:grid-cols-12 md:items-end">
            ...
            <div className="md:col-span-4">
              <p className="max-w-md text-sm leading-6 text-white/70 md:text-[15px]">A category-led architecture archive in the same visual language as the interior page, with hover zoom on every project card.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-6 md:px-10 md:py-8">
        <div className="mx-auto flex max-w-7xl flex-wrap gap-3">
          {architectureCategories.map((category) => (
            <button key={category.slug} type="button" onClick={() => setActiveCategory(category.slug)} className={`rounded-full border px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition-all ${activeCategory === category.slug ? 'border-black bg-black text-white' : 'border-black/20 bg-white/80 text-black/85 hover:border-black/45 hover:text-black'}`}>
              {category.label}
            </button>
          ))}
        </div>
