...
function SiteHeader({ menuOpen, setMenuOpen, mobileSection, toggleSection, mobileProject, setMobileProject }) {
  return (
    <header className="fixed inset-x-0 top-0 z-50 text-white">
      <div className="nav-shell mx-4 mt-4 flex h-14 items-center justify-between rounded-full border border-white/20 bg-[#11110f]/90 px-5 md:mx-7 md:mt-5 md:h-16 md:px-7">
        ...
        <nav className="hidden h-full items-center gap-8 md:flex">
          ...
          <Link href="/contact" className="contact-pill inline-flex items-center gap-2 rounded-full border border-white/35 px-5 py-2.5 text-[10px] font-semibold uppercase tracking-[0.16em] transition-colors hover:bg-white hover:text-[#11110f]">
            Contact us <Arrow />
          </Link>
        </nav>
        ...
        <div className="mx-4 mt-2 max-h-[80vh] overflow-y-auto rounded-[24px] border border-white/15 bg-[#11110f] p-5 text-white shadow-2xl md:hidden">
          ...
          {mobileSection === 'projects' && <div className="pl-3">
            {projectDisciplines.map((discipline) => <div key={discipline.title} className="border-b border-white/10">
              ...
              {mobileProject === discipline.title && <div className="grid grid-cols-2 gap-x-4 pb-3 text-[11px] text-white/55">{discipline.children.map(item => <Link key={item} href={discipline.title === 'Interiors' ? '/interior' : item === 'All' ? '/architecture' : `/architecture/${item.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`} onClick={() => setMenuOpen(false)} className="py-2">{item}</Link>)}</div>}
            </div>)}
          </div>}
          <div>
            ...
            {mobileSection === 'about' && <div className="pl-3">{aboutMenu.map(([title]) => <Link key={title} href="#about" onClick={() => setMenuOpen(false)} className="flex items-center justify-between border-b border-white/10 py-3 text-[14px] text-white/70">{title}<Arrow /></Link>)}</div>}
          </div>
          <Link href="/contact" onClick={() => setMenuOpen(false)} className="mt-5 flex items-center justify-between rounded-full border border-white/30 px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.16em]">
            Contact us <Arrow />
          </Link>
        </div>
      )}
    ...
    <footer id="contact" className="bg-[#cfc7b8] px-5 pb-7 pt-14 md:px-10 md:pb-9 md:pt-16">
      <div className="grid gap-10 md:grid-cols-12 md:gap-8">
        <div className="md:col-span-7">
          ...
          <h2 className="mt-5 text-[13vw] leading-[0.82] tracking-[-0.07em] md:text-[7.5vw]">Have a site?<br /><span className="serif italic text-black/50">Letâ€™s talk.</span></h2>
        ...
        <div className="md:col-span-5 md:pt-1">
          <div className="border-b border-black/25 pb-7">
            ...
            <p className="mt-4 text-[9px] uppercase tracking-[0.16em] text-black/45">Private Residential Client Â· Mumbai</p>
          </div>
          <div className="pt-7">
      ...
      <div className="mt-12 grid gap-8 border-t border-black/20 pt-6 md:grid-cols-12 md:items-end">
        ...
        <p className="text-[9px] uppercase tracking-[0.16em] text-black/50 md:col-span-3 md:text-right">Â© 2026 AXIS Architecture. All rights reserved.</p>
      </div>
    </footer>
  );
}
      ...
      <section className="px-5 py-14 md:px-10 md:py-18">
        <div className="mx-auto max-w-4xl">
          <div className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-black/60 md:text-[15px]">
              Whether it&apos;s architecture, interiors, or a new collaboration, start here and we&apos;ll take it from there.
            </p>
          </div>
          ...
          <form className="mt-10 rounded-[32px] border border-black/10 bg-[#ece6db] p-5 shadow-[0_20px_60px_rgba(17,17,15,0.08)] md:p-8">
            <div className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50">
                Name
                <input className="border-b border-black/20 bg-transparent py-3 text-base outline-none placeholder:text-black/30" type="text" placeholder="Your name" />
              </label>
              <label className="grid gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50">
                Email
                <input className="border-b border-black/20 bg-transparent py-3 text-base outline-none placeholder:text-black/30" type="email" placeholder="you@example.com" />
              </label>
            </div>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <label className="grid gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50">
                Project type
                <input className="border-b border-black/20 bg-transparent py-3 text-base outline-none placeholder:text-black/30" type="text" placeholder="Architecture, interiors, advisory..." />
              </label>
              <label className="grid gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50">
                Timeline
                <input className="border-b border-black/20 bg-transparent py-3 text-base outline-none placeholder:text-black/30" type="text" placeholder="When are you planning to begin?" />
              </label>
            </div>
            <label className="mt-4 grid gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50">
              Message
              <textarea className="min-h-[180px] border-b border-black/20 bg-transparent py-3 text-base outline-none placeholder:text-black/30" placeholder="Tell us about your site, location, goals, and anything else we should know." />
            </label>
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-black/10 pt-5">
              ...
              <button type="submit" className="contact-pill inline-flex items-center gap-2 rounded-full border border-black/35 px-5 py-3 text-[10px] font-semibold uppercase tracking-[0.16em] transition-colors hover:bg-black hover:text-white">
                Send message <Arrow />
              </button>
            </div>
          </form>
