...
export default function ProjectsPage() {
  const mixedProjects = useMemo(
    () => [
      ...architectureProjects.map((project) => ({ ...project, group: 'Architecture' })),
    ...
    <main className="min-h-screen bg-[#f4efe6] text-[#11110f]">
      <section className="relative overflow-hidden border-b border-black/10 bg-[#11110f] px-5 pb-12 pt-24 text-white md:px-10 md:pb-16">
        ...
        <div className="relative mx-auto max-w-7xl">
          <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-white/55">Projects / All projects</p>
          <div className="mt-5 grid gap-6 md:grid-cols-12 md:items-end">
            ...
            <div className="md:col-span-4">
              <p className="max-w-md text-sm leading-6 text-white/70 md:text-[15px]">
                A single entry point for the full archive. Use the pills to jump into the dedicated architecture or interiors archives.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap gap-3">
          {categories.map((category) => (
            <Link
              key={category.label}
              href={category.href}
              className="rounded-full border border-black/15 bg-white/70 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/70 transition-all hover:border-black/30 hover:bg-black hover:text-white focus:bg-black focus:text-white active:bg-black active:text-white"
            >
              {category.label}
            </Link>
          ))}
      ...
      <section className="px-5 pb-16 md:px-10 md:pb-20">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-12 md:gap-6">
          {mixedProjects.map((project, index) => (
            <div key={`${project.group}-${project.title}-${project.location}`} className={`${index % 5 === 0 || index % 5 === 3 ? 'md:col-span-7' : 'md:col-span-5'}`}>
